...
/*onmousedown = function(e){
	document.getElementById("bar").style.left = e.clientX-10+'px';
	document.getElementById("bar").style.top = e.clientY-18+'px';
	console.log(document.getElementById("editor").getBoundingClientRect());
}
*/
...
edit.addEventListener('keydown', () => {
	...
	var key = keypress;
	/*if (keypress === 16) {
		document.getElementById("editor").innerHTML.substr(0, getCaret()) + "nfkjk.fn" + document.getElementById("editor").innerHTML.substr(getCaret);
	}
	*/console.log(getCaret());
	/*
	if (keypress === 13) {
		document.getElementById("editor").innerHTML += "&#9;";
		// need to insert at cursor not start of string !!!!
	}*/
	// cause the newline issues ^^^
	if (keypress === 9) {
		event.preventDefault();
		edit.focus()
		document.execCommand('insertText', false /*no UI*/, String.fromCharCode('&Tab;'));
...
function getCaret() {
	...
	// clear selection
	window.getSelection().collapseToEnd();
	var caret = text.length - 2;
	//console.log(`caret: ${text.length - 2}, line: ${lines}`);
	return caret;
	setCookie('cursor', ((lines) + "." + (text.length - 2)));
}

function setCaret() {
		var index = getCookie('cursor').split(".");
    var el = document.getElementById("editable")
    var range = document.createRange()
    var sel = window.getSelection()
    
    range.setStart(el.childNodes[index[0]], index[1])
    range.collapse(true)
